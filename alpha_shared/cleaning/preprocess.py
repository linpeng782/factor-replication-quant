...
def prepare_factor(
    ...
    """
    完整清洗链路：can_buy_mask → winsorize → standardize → not_limit_up_mask

    实盘时序逻辑:
      1. T 日盘后: 用 can_buy_mask 过滤 ST / 停牌 / 新股
         （这三类 T 日就已知不可交易，提前剔除避免污染横截面分布）
      2. T 日盘后: 在干净的横截面上做 winsorize_mad + standardize_zscore
         （涨停股仍参与，因为它们是正常市场信号）
      3. T+1 日开盘前: 用 not_limit_up_mask 过滤涨停
         （涨停股 T+1 日不能买入，最终从可交易集合里剔除）

    参数:
        factor   : (T, N) 原始因子面板
        can_buy_mask : (T, N) 布尔 mask，True = 参与因子分布
                   = NOT is_st AND NOT is_suspended AND NOT is_new_stock
        not_limit_up_mask: (T, N) 布尔 mask，True = 可执行下单
                   = NOT is_limit_up
        mad_n    : MAD 去极值阈值倍数，默认 3

    返回:
        (T, N) 清洗后的因子面板（最终只在 can_buy_mask & not_limit_up_mask 处有值）
    """
    # 对齐索引和列（以 can_buy_mask 为基准）
    factor = factor.reindex(index=can_buy_mask.index, columns=can_buy_mask.columns)
    ...
    factor = factor.where(can_buy_mask)
    ...
    factor = factor.where(not_limit_up_mask)
    n_final = factor.notna().values.sum()

    logger.info(
        f"[preprocess] 清洗完成: 原始={n_raw:,} → "
        f"can_buy_mask 后={n_after_pre:,} → 标准化后={n_after_std:,} → "
        f"not_limit_up_mask 后={n_final:,}"
    )
    return factor
