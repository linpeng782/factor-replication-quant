...
详见 `docs/minute_factor_replication.md`（数据路径、`minute_intraday_aggregate` 算子、cache 机制、`compute` 的 NaN 惯用法、ridge 缺失率坑等）。

**最常用三件事**：
- 分钟数据 = `MINUTE_DATA_DIR`；spec 用 `universe.primary_index: MINUTE_DIR` 自动扫股票池
- 128 核机器跑全市场：`MINUTE_WORKERS=100 python run.py <factor>`（~50 秒，**不要用线程池版**）
- 改算子 superset 列定义时 bump `_FEATURES_SUPERSET_VERSION`；只想强制重算时改 spec 的 `cache_key`（`prv_v1` → `prv_v2`）

模板：`sources/kysec/paper_27_microstructure/specs/peak_minute_count/`（最简）；`.../specs/peak_interval_kurt/`（高阶矩 + 守门 NaN）。
