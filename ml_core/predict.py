...
def _load_meta(model_dir: Path) -> dict:
    ...
    logger.warning(f"[predict] {model_dir.name} 无 run_meta.json，按老产物回退拼装（model={model}）")
    return {"model": model, "sources": sources, "neu_sources": neu,
            "has_factor_policy": "none" if model == "lgbm" else "all",
            "horizon": 20, "select_method": None, "feature_order": feats}
...
def _pathmap(meta: dict) -> dict:
    ...
    neu = discover_features(meta["neu_sources"], stage="neu") if meta.get("neu_sources") else {}
    return {**neu, **raw}


def _factor_end(pathmap: dict, feats: list[str]) -> pd.Timestamp:
    """入选因子「共同覆盖」的最末交易日（读 index 不读数据，廉价）。"""
    ends = [pd.to_datetime(pd.read_parquet(pathmap[f], columns=[]).index).max() for f in feats]
...
def _resolve_window(pc: dict, meta: dict) -> tuple[str | None, str]:
    ...
    if pc.get("latest_n"):
        cal = pd.to_datetime(pd.read_parquet(pathmap[feats[0]], columns=[]).index).sort_values()
        cal = cal[cal <= end]
        start = cal[-int(pc["latest_n"])]
        logger.info(f"[predict] latest_n={pc['latest_n']} → 区间 {start.date()}~{end.date()}（日更增量）")
        return str(start.date()), str(end.date())
    logger.info(f"[predict] 全量区间 {pc.get('start') or '*'}~{end.date()}")
    return pc.get("start"), str(end.date())


def _coverage_guard(panel: pd.DataFrame, recent: int, drop: float, strict: bool) -> None:
    ...
    bad = [(d.date(), int(n)) for d, n in zip(panel.index[-recent:], nn[-recent:])
           if baseline > 0 and n < drop * baseline]
    if bad:
        logger.warning(f"[predict][coverage] ⚠️ 近 {recent} 日非空骤降（基线~{int(baseline)}/日）：{bad}")
        if strict:
            raise RuntimeError("[predict][coverage] 覆盖骤降且 strict_coverage=true，中止以防污染信号")
    else:
        logger.info(f"[predict][coverage] ✅ 近 {recent} 日覆盖正常（基线~{int(baseline)}/日）")
...
def _run(pc: dict) -> None:
    ...
    logger.info(f"=== ml_core 推理 model_run_id={run_id} | 模型={meta['model']} "
    ...
    cfg = PipelineConfig(sources=meta["sources"], neu_sources=meta.get("neu_sources"),
                         has_factor_policy=policy, horizon=meta["horizon"],
