...
### 已复现因子详情（新算子契约下，2026-05-20 全量复跑）

> 评估区间统一为 `2016-01-04 ~ 2025-12-31`。md5 列对照 `.bak`（旧版工作树产物）：
> ✅ 字节相等 = 算子语义零偏差；⚠️ 不等 = 修了上游字段或老 .bak 本身就有问题。

| 因子名 | 复现 IC | 复现 ICIR | 单调性 | LongShort Sharpe | cleaned md5 vs .bak | 备注 |
|--------|--------|----------|--------|-----------------|---------------------|------|
| pe_ttm_new       | 0.0148 | 0.155 | +0.772 | 0.970 | ✅ | 单 fetch + 改名 |
| pe_ttm_delta60   | 0.0245 | 0.330 | +0.801 | 1.911 | ✅ | fetch + 60 日 diff |
| pe_mrq           | 0.0133 | 0.163 | +0.660 | 0.946 | ⚠️ | 字段切到 `market_cap_3`，IC 差异 < 1% |
| roe_mrq_new      | 0.0160 | 0.127 | +0.978 | 0.384 | ⚠️ | 首次正确 YOLO（旧 .bak/raw 是 158KB 残次品） |
| roe_pyoy_mrq     | 0.0196 | 0.242 | +0.971 | 1.592 | ⚠️ | 首次完整 5 步链路（旧 .bak 是共用文件） |
| roe_pqoq_mrq     | 0.0118 | 0.182 | +0.889 | 1.012 | ✅ | qoq 版本 + 滤分母>0 |
| roe_ayoy_mrq     | 0.0157 | 0.228 | +0.951 | 1.636 | ✅ | abs 分母版本，不过滤 |
| roe_apoq_mrq     | 0.0066 | 0.141 | +0.756 | 0.890 | ✅ | abs 分母 qoq，不过滤 |

### 旧记录差异说明（已被本轮覆盖）

- `pe_ttm_delta60` 之前 PROGRESS 记 IC 0.0148 / ICIR 0.155（标 ⚠️ diff 未生效）—— 这是更早的 bug，
  本轮 IC 0.0245 / ICIR 0.330 是 transform diff 真正生效后的结果，参见 `docs/roic_ttm_issues.md` 与 commit `847192a`。
- `roe_mrq_new` / `roe_pyoy_mrq` 旧记录是同一份共用 cleaned 评估出来的（PROGRESS 第 97 行原注），
  本轮是各自独立 YOLO 全链路第一次产出。
...
| P2 | npf 系列（pyoy/pqoq/ayoy/apoq） | ROE 模板平移即可 |
| P3 | roic_ttm 三因子（ind_rnk8 / all_rnk8 / dev_std8） | 需要 merge + change-day rolling，留到所有简单因子验证完成后 |
| P3 | 适度冒险因子（moderate_risk） | 需要分钟级数据 + 截面去均值算子，需扩展算子库 |
| P3 | 辅助指标 | 非核心因子 |
| P3 | 辅助指标 | 非核心因子，低优先级 |
