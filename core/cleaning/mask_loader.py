...
数据源:
    1. config.COMBO_MASK_PATH（由 backtest_engine 项目产出）
       长表，列 [order_book_id, datetime, is_st, is_suspended, is_limit_up]
    2. config.NEW_STOCK_MASK_PATH（由 backtest_engine 产出）
...
def load_filter_masks(
    ...
    参数:
        ...
        combo_mask_path      : combo_mask 文件路径，None 时使用 config.COMBO_MASK_PATH
        new_stock_mask_path  : new_stock_mask 文件路径，None 时使用 config.NEW_STOCK_MASK_PATH

    返回:
        (pre_mask, post_mask): tuple[pd.DataFrame, pd.DataFrame]
    """
    combo_path = Path(combo_mask_path or config.COMBO_MASK_PATH)
    new_stock_path = Path(new_stock_mask_path or config.NEW_STOCK_MASK_PATH)
    ...
    logger.info(
        f"[filters] post_mask 通过率={post_mask.values.mean():.2%} " f"(过滤涨停)"
    )
    logger.info(
        f"[filters] 最终可交易比例={final.values.mean():.2%} " f"(pre_mask & post_mask)"
    )
