...
def compute_corr(factors: list[str], pathmap: dict, workers: int = 16):
    ...
    def work(f):
        df = pd.read_parquet(pathmap[f]); df.index = pd.to_datetime(df.index); df = df.loc[START:END]
        ...
        s = float(ic.std()); icir = abs(float(ic.mean()) / s) if s else 0.0
        return f, a, ic, icir

    logger.info(f"  并行读 {len(factors)} 因子（{workers} 线程，单读：值采样+日IC+方向+ICIR）...")
    res = {}; icir = {}
    with ThreadPoolExecutor(max_workers=workers) as ex:
        for f, a, ic, iciv in ex.map(work, factors):
            res[f] = (a, ic); icir[f] = iciv
    ...
    for i, f in enumerate(factors):
        a = res[f][0]; m = ~np.isnan(a); a = np.where(m, a, np.float32(0.0))
