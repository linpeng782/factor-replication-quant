"""
因子: npf_pqoq_mrq — 单季度净利润环比（分母 > 0 版）

定义：(net_profit_mrq_0 - net_profit_mrq_1) / abs(net_profit_mrq_1)，仅保留 mrq_1 > 0 样本
方向：正向；分类：景气
"""
...
def main():
    ...
    factor = factor.where(np_1 > 0)   # 过滤分母 > 0

    print(f"因子 shape: {factor.shape}, 非空率 {factor.notna().mean().mean():.2%}")
    factor.to_parquet("npf_pqoq_mrq.parquet")
