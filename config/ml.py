...
代码线（repo 顶层 ml_core/ 等）与数据线（此处，数据根下）分离。
★ 这些常量只被 ML 训练线（ml_core/ 等）消费；因子生产线不碰。
  依赖方向单向：ML 读因子产物（factors.py 的 RAW/NEU_FACTOR_BASE）+ 评估输入（market_data.py 的 mask/labels），
  反之因子线不依赖这里。
"""

from .base import _DATA_ROOT, _REPO_ROOT, ML_HT_BACKEND
