...
### 第③层 — 入库日频 IC【run.py 评估输出，区间 2016-2025（对齐 output 评估 PNG）】
`run.py guosen/co_momentum/<f>` 各因子 neu 版（direction 已自动判定，VICR 为 −1）；IC=spearman，ICIR=mean/std 未年化：

| 因子 | IC5d | IC10d | IC20d | ICIR(5d / 20d) | 单调性 mono |
|------|------|-------|-------|----------------|-------------|
| ICM  | 4.34% | 4.72% | 5.14% | 0.79 / 1.00 | 0.95 |
| VICM | 4.54% | 4.96% | 5.45% | 0.83 / 1.05 | 0.94 |
| VICR | 4.48% | 4.88% | 5.44% | 0.76 / 0.98 | 0.93 |
| CMC  | 4.61% | 5.03% | 5.58% | 0.80 / 1.03 | 0.94 |
| MCMC | 5.01% | 5.53% | 6.16% | 0.75 / 0.98 | 0.91 |

→ 服务器对照：**方向全为正（VICR dir=−1 翻正后）、单调性 > 0.90、IC5d≈4.3~5%、IC20d≈5.1~6.2%**。
   可对评估 PNG 核验，例 CMC neu：5d=0.0461 / 20d=0.0558 / ICIR20=1.03 / 多空Sharpe≈5.04。
...
| ③ 日频 IC | 方向全正、mono>0.90、IC20d 5.1~6.2% | 量级级，不强求 bit |
