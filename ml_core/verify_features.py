...
============================================================
运行（dquant 后端）：
    ALPHA158_DATA_BACKEND=dquant \
        python -m ml_core.verify_features

检查：
  1. discover：alpha158-dquant 应 158 个；kysec-dquant 应能发现 paper_27。
...
def main() -> None:
    ...
    logger.info(f"  alpha158-dquant={len(a158)} | kysec-dquant={len(kysec)}")
    assert len(a158) == 158, f"❌ alpha158-dquant 应 158 个，实得 {len(a158)}"
    assert len(kysec) > 0, "❌ kysec-dquant 未发现因子"
    logger.success("  ✅ discover 正常")
