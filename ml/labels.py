...
============================================================
  ...
  超额     = r − 当日 can_buy_mask universe 的【等权均值】     （减市场 = 定义超额）
  标准化   = 对 excess 再套全集 RobustZScore（在 dataset/preprocess 阶段统一处理）

注：demean 用 can_buy_mask universe（剔 ST/停牌/新股；涨停股保留，是有效信号）。
...
def build_excess_label(
    ret: pd.DataFrame,
    can_buy_mask: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """截面等权 demean → 超额收益面板（与 ret 同 (T, N) 网格）。

    mkt[t] = mean_j( ret[t, j] | can_buy_mask[t, j] )   # 当日可投 universe 等权市场收益
    excess = ret.sub(mkt, axis=0)                    # 对【全体】股票减去当日市场（保留全网格）

    参数：
        ret      : (T, N) 绝对收益面板
        can_buy_mask : (T, N) 布尔；True=计入市场均值。None 则用 ret 全部非 NaN。
    """
    if can_buy_mask is not None:
        can_buy_mask = can_buy_mask.reindex(index=ret.index, columns=ret.columns)
        ret_for_mean = ret.where(can_buy_mask.astype("boolean").fillna(False))
    else:
        ret_for_mean = ret
