...
定义（双层截面回归）：
  ...
  log_npttm = log(net_profit_ttm_0)                          TTM 净利润对数
...
预过滤：保留 net_profit_mrq_0 > 0 & net_profit_mrq_4 > 0 & net_profit_ttm_0 > 0 & pe_ratio_ttm > 0
...
def main():
    ...
    print("拉 4 个原料字段 ...")
    fields = ["net_profit_mrq_0", "net_profit_mrq_4", "net_profit_ttm_0", "pe_ratio_ttm"]
    ...
    df = df.query(
        ...
        "and net_profit_ttm_0 > 0 and pe_ratio_ttm > 0"
    ).reset_index(drop=True)

    # log
    df["log_yoy"]   = np.log(df["net_profit_mrq_0"] / df["net_profit_mrq_4"])
    df["log_npttm"] = np.log(df["net_profit_ttm_0"])
